...
def load_settings():
    """Reads settings from the server disk safely."""
...
def save_settings():
    """Saves current widget states to the shared JSON file."""
    # We use .get to safely grab the state even if widgets aren't rendered yet
    settings = {
        "tickers": st.session_state.get("tickers", ["SPY", "AMZN", "NVDA", "GOOGL"]),
...
if "init_loaded" not in st.session_state:
    saved = load_settings()
    if saved:
        # Load into Session State keys that match widget 'key' parameters
...
# Using 'key' in sidebar widgets automates the link to session_state
...
if st.sidebar.button("🚨 EMERGENCY SELL ALL", type="primary", use_container_width=True):
    try:
        ...
        st.session_state.logs.append(f"🚨 EMERGENCY: Liquidation at {datetime.now().strftime('%H:%M:%S')}")
        save_settings()
        st.rerun()
...
def scan_ticker(symbol, run_bot, mode, val):
    try:
        ...
        if run_bot and prob >= 0.90:
            ...
            st.session_state.logs.append(f"🤖 AUTO BUY: {symbol} @ {price:.2f} (AI: {prob*100:.1f}%)")
            save_settings()
...
def trading_dashboard():
    # SYNC FROM FILE: Look for changes from other devices without breaking widgets
    global_data = load_settings()
    if global_data:
        # We only sync 'logic' variables here, we don't touch widget keys to avoid the Exception
        active_tickers = global_data.get("tickers", st.session_state.tickers)
        bot_logic_active = global_data.get("run_bot", st.session_state.bot_toggle)
    else:
        active_tickers = st.session_state.tickers
        bot_logic_active = st.session_state.bot_toggle
    ...
    try:
        ...
        # Risk protection override
        if pnl >= st.session_state.goal_input or pnl <= -abs(st.session_state.loss_limit):
            bot_logic_active = False
    except: pass
