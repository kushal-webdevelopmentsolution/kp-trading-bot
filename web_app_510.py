...
with st.sidebar.expander("➕ Add New Symbol", expanded=False):
    new_ticker = st.text_input("Enter Symbol").upper().strip()
...
# Remove Ticker
updated_tickers = st.sidebar.multiselect("Current Watchlist", options=st.session_state.tickers, default=st.session_state.tickers)
...
profit_goal = st.sidebar.number_input("Daily Profit Goal ($)", min_value=1.0, value=200.0)
loss_limit = st.sidebar.number_input("Daily Max Loss ($)", min_value=1.0, value=100.0)
...
if st.session_state.logs:
    log_df = pd.DataFrame(st.session_state.logs, columns=["Trade Activity Log"])
    csv = log_df.to_csv(index=False).encode('utf-8')
    st.sidebar.download_button(
        label="📥 Download Trade Report",
        data=csv,
        file_name=f"trade_report_{datetime.now().strftime('%Y%m%d')}.csv",
        mime='text/csv',
    )
...
if st.sidebar.button("🚨 EMERGENCY SELL ALL", type="primary", use_container_width=True):
    try:
        ...
        st.sidebar.error(f"Exit error: {e}")

# --- CLOUD-SAFE RISK ENGINE ---
...
def get_cloud_risk_matrix(tickers):
    ...
    try:
        ...
        for symbol in tickers:
            req = StockBarsRequest(symbol_or_symbols=symbol, timeframe=TimeFrame.Day, start=start_date, end=end_time, feed=DataFeed.IEX)
...
def scan_ticker(symbol, run_bot, order_mode, order_val):
    try:
        end_time = datetime.now() - timedelta(minutes=20)
        req = StockBarsRequest(symbol_or_symbols=symbol, timeframe=TimeFrame.Day, start=datetime.now()-timedelta(days=730), end=end_time, feed=DataFeed.IEX)
        df = data_client.get_stock_bars(req).df.reset_index()
        ...
        if run_bot and prob >= 0.90:
            ...
            st.session_state.logs.append(f"🤖 AI BUY: {symbol} @ {price:.2f} (Conf: {prob*100:.0f}%)")
...
def trading_dashboard():
    ...
    try:
        ...
        if pnl >= profit_goal:
            st.success("💰 Profit Goal Reached!"); run_bot_active = False
        elif pnl <= -abs(loss_limit):
            st.error("🛑 Loss Limit Hit!"); run_bot_active = False
        else:
            run_bot_active = run_bot
    except:
        st.warning("Connecting to Alpaca..."); run_bot_active = False

    # 2. Holdings View
    with st.expander("💼 Active Positions", expanded=True):
        pos = trading_client.get_all_positions()
        if pos:
            df_pos = pd.DataFrame([{"Ticker": p.symbol, "Qty": p.qty, "Price": f"${float(p.current_price):.2f}", "PnL": f"${float(p.unrealized_pl):.2f}"} for p in pos])
    ...
    if st.session_state.tickers:
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as executor:
            futures = [executor.submit(scan_ticker, s, run_bot_active, order_mode, order_val) for s in st.session_state.tickers]
        ...
        for res in results:
            ...
            if r[3].button(f"Buy {res['qty']}", key=f"b_{res['symbol']}"):
                st.session_state.logs.append(f"👤 Manual BUY: {res['symbol']} @ {res['price']}")
        ...
        with right:
            ...
            else: st.info("Risk data syncing...")
